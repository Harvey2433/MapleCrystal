...
import top.maple_bamboo.maplecrystal.mod.gui.items.buttons.Button;
import top.maple_bamboo.maplecrystal.mod.gui.items.buttons.ModuleButton;
    ...
    public void drawScreen(DrawContext context, int mouseX, int mouseY, float partialTicks) {
        ...
        if (this.open) {
            if (ClickGui.getInstance().blur.getValue()) {
                MapleCrystal.BLUR.applyBlur(1.0f + (ClickGui.getInstance().radius.getValueFloat() - 1.0f) * (float)ClickGui.getInstance().alphaValue, this.x, (float)this.y + (float)this.height - 5.0f, this.width, totalItemHeight + 5.0f);
            }
