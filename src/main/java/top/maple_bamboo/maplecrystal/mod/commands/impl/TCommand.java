...
extends Command {
    ...
    public void runCommand(String[] parameters) {
        ...
        Module module = MapleCrystal.MODULE.getModuleByName(moduleName);
        if (module == null) {
    ...
    public String[] getAutocorrect(int count, List<String> seperated) {
        if (count == 1) {
            ...
            for (Module x : MapleCrystal.MODULE.getModules()) {
                if (!input.equalsIgnoreCase(MapleCrystal.getPrefix() + "t") && !x.getName().toLowerCase().startsWith(input)) continue;
