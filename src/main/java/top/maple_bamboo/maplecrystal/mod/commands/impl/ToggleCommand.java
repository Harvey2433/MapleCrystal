...
extends Command {
    ...
    public void runCommand(String[] parameters) {
        ...
        Module module = Config.MODULE.getModuleByName(moduleName);
        if (module == null) {
    ...
    public String[] getAutocorrect(int count, List<String> seperated) {
        if (count == 1) {
            ...
            for (Module x : Config.MODULE.getModules()) {
                if (!input.equalsIgnoreCase(Config.getPrefix() + "toggle") && !x.getName().toLowerCase().startsWith(input)) continue;
