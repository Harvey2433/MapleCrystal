...
import top.maple_bamboo.maplecrystal.mod.modules.impl.misc.FakePlayer;
...
extends Command {
    ...
    public String[] getAutocorrect(int count, List<String> seperated) {
        if (count == 1) {
            ...
            for (String x : list) {
                if (!input.equalsIgnoreCase(MapleCrystal.getPrefix() + "fakeplayer") && !x.toLowerCase().startsWith(input)) continue;
                correct.add(x);
            }
