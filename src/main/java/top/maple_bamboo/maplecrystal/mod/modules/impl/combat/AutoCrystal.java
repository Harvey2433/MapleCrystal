...
import top.maple_bamboo.maplecrystal.api.utils.combat.CombatUtil;
import top.maple_bamboo.maplecrystal.api.utils.entity.PlayerEntityPredict;
import top.maple_bamboo.maplecrystal.api.utils.math.*;
...
import top.maple_bamboo.maplecrystal.asm.accessors.IEntity;
...
import top.maple_bamboo.maplecrystal.mod.modules.impl.exploit.Blink;
import top.maple_bamboo.maplecrystal.mod.modules.impl.movement.ElytraFly;
...
import top.maple_bamboo.maplecrystal.mod.modules.settings.enums.SwingSide;
import top.maple_bamboo.maplecrystal.mod.modules.settings.enums.Timing;
...
extends Module {
    ...
    public AutoCrystal() {
        ...
        INSTANCE = this;
        MapleCrystal.EVENT_BUS.subscribe(new CrystalRender());
    }
...
//        }
    ...
    private void calcCrystalPos() {
        ...
        if (list.isEmpty()) {
            ...
            float selfDamage;
            Vec3d attackVec;
            for (Entity entity : MapleCrystal.THREAD.getEntities()) {
            ...
            for (BlockPos pos : BlockUtil.getSphere((float)this.breakRange.getValue() + 1.5f)) {
                ...
                if (base && MapleCrystal.BREAK.isMining(pos.down()) && this.detectMining.getValue() || (attackVec = this.getAttackVec(pos.toBottomCenterPos())) == null || this.behindWall(pos, attackVec) || !this.canTouch(pos.down()) || !this.canPlaceCrystal(pos, true, false)) continue;
    ...
    private void doPlace(BlockPos pos) {
        ...
        if (this.detectMining.getValue() && MapleCrystal.BREAK.isMining(pos)) {
            return;
        }
    ...
    private void doBreak(EndCrystalEntity entity) {
        ...
        if ((crystalPos = this.crystalPos) != null && this.displayTarget != null && (double)this.lastDamage >= this.getDamage(this.displayTarget) && this.afterBreak.getValue() && (!this.rotate.getValue() || !this.shouldYawStep() || !this.checkFov.getValue() || MapleCrystal.ROTATION.inFov(entity.getPos(), this.fov.getValueFloat()))) {
    ...
    private boolean faceVector(Vec3d directionVec) {
        ...
        if (!this.shouldYawStep()) {
            MapleCrystal.ROTATION.lookAt(directionVec);
            return true;
        }
        this.directionVec = directionVec;
        if (MapleCrystal.ROTATION.inFov(directionVec, this.fov.getValueFloat())) {
            return true;
        }
