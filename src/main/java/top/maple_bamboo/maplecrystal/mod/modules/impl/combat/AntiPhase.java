...
extends Module {
                ...
                if (this.fill.getValue() && itemFrameEntity != null && itemFrameEntity.getHeldItemStack().isEmpty()) {
                    ...
                    if (AntiPhase.mc.player.getEyePos().distanceTo(hitVec) <= AntiCheat.INSTANCE.ieRange.getValue() && (block3 = this.getObsidian()) != -1) {
                        ...
                        if (this.rotate.getValue()) {
                            MapleCrystal.ROTATION.snapAt(hitVec);
                        }
    ...
    private Direction targetFacing(Vec3d vec3d) {
        ...
        for (Direction direction : Direction.values()) {
            ...
            double getDistance;
            BlockPos bp;
            if (direction == Direction.UP || direction == Direction.DOWN || !BlockUtil.isStrictDirection(bp = blockPos.offset(direction), direction.getOpposite()) || (!this.onlyHard.getValue() ? BlockUtil.canReplace(bp) || !BlockUtil.canClick(bp) : !MapleCrystal.HOLE.isHard(bp))) continue;
