...
extends Module {
    ...
    private String getNametagInfo(PlayerEntity player) {
        ...
        if (this.totemsConfig.getValue() && (totems = MapleCrystal.POP.getPop(player)) > 0) {
    ...
    private int getNametagColor(PlayerEntity player) {
        if (this.friendConfig.booleanValue && player.getDisplayName() != null && MapleCrystal.FRIEND.isFriend(player)) {
