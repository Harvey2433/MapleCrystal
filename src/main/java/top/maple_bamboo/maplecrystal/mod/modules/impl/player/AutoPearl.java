...
import top.maple_bamboo.maplecrystal.api.events.impl.TickEvent;
...
extends Module {
    ...
    public void onUpdate(TickEvent event) {
        ...
        if (!this.shouldYawStep()) {
            this.throwPearl(AutoPearl.mc.player.getYaw(), AutoPearl.mc.player.getPitch());
            this.disable();
        } else if (MapleCrystal.ROTATION.inFov(AutoPearl.mc.player.getYaw(), AutoPearl.mc.player.getPitch(), this.fov.getValueFloat())) {
            if (this.sync.getValue()) {
                ...
                if (AutoPearl.mc.player.getMainHandStack().getItem() == Items.ENDER_PEARL) {
                    AutoPearl.sendSequencedPacket(id -> new PlayerInteractItemC2SPacket(Hand.MAIN_HAND, id, MapleCrystal.ROTATION.getLastYaw(), MapleCrystal.ROTATION.getLastPitch()));
    ...
    public void throwPearl(float yaw, float pitch) {
        ...
        if (AutoPearl.mc.player.getMainHandStack().getItem() == Items.ENDER_PEARL) {
            if (this.rotation.getValue()) {
                MapleCrystal.ROTATION.snapAt(yaw, pitch);
            }
