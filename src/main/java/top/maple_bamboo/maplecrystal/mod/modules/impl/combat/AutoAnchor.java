...
import top.maple_bamboo.maplecrystal.api.events.impl.RotationEvent;
...
import top.maple_bamboo.maplecrystal.api.utils.math.AnimateUtil;
import top.maple_bamboo.maplecrystal.api.utils.math.ExplosionUtil;
...
import top.maple_bamboo.maplecrystal.mod.modules.impl.player.AirPlace;
...
extends Module {
    ...
    public AutoAnchor() {
        ...
        INSTANCE = this;
        MapleCrystal.EVENT_BUS.subscribe(new AnchorRender());
    }
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if ((pos = this.currentPos) != null) {
            ...
            boolean bl = shouldSpam = this.spam.getValue() && (!this.mineSpam.getValue() || MapleCrystal.BREAK.isMining(pos));
            if (shouldSpam) {
    ...
    public BlockPos getHelper(BlockPos pos) {
        for (Direction i : Direction.values()) {
            if (this.checkMine.getValue() && MapleCrystal.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i))) continue;
            return pos.offset(i);
        }
        return null;
    }
