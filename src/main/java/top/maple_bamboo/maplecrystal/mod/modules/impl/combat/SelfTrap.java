...
import top.maple_bamboo.maplecrystal.api.events.impl.MoveEvent;
...
import top.maple_bamboo.maplecrystal.api.utils.player.MovementUtil;
import top.maple_bamboo.maplecrystal.api.utils.world.BlockUtil;
import top.maple_bamboo.maplecrystal.core.impl.BreakManager;
...
extends Module {
    ...
    public SelfTrap() {
        ...
        INSTANCE = this;
        MapleCrystal.EVENT_BUS.subscribe(new SelfTrapTick());
    }
    ...
    private void doSurround(BlockPos pos) {
        for (Direction i : Direction.values()) {
            ...
            if (this.godMode.getValue()) {
                for (BreakManager.BreakData breakData : MapleCrystal.BREAK.breakMap.values()) {
    ...
    public BlockPos getHelperPos(BlockPos pos) {
        for (Direction i : Direction.values()) {
            if (this.detectMining.getValue() && MapleCrystal.BREAK.isMining(pos.offset(i)) || !BlockUtil.isStrictDirection(pos.offset(i), i.getOpposite()) || !BlockUtil.canPlace(pos.offset(i))) continue;
            return pos.offset(i);
        }
        return null;
    }
