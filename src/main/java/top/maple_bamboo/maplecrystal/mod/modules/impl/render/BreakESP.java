...
extends Module {
    ...
    private Color getFillColor(PlayerEntity player) {
        return MapleCrystal.FRIEND.isFriend(player) ? this.fillFriend.getValue() : this.fill.getValue();
    }

    private Color getBoxColor(PlayerEntity player) {
        return MapleCrystal.FRIEND.isFriend(player) ? this.boxFriend.getValue() : this.box.getValue();
    }
    ...
    public void onRender3D(MatrixStack matrixStack) {
        ...
        if (this.second.getValue()) {
            Iterator<BreakManager.BreakData> iterator = ((ConcurrentHashMap.KeySetView) MapleCrystal.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)((Object)iterator.next());
                BreakManager.BreakData breakData = MapleCrystal.BREAK.doubleMap.get(i);
                if (breakData == null || breakData.getEntity() == null || BreakESP.mc.world.isAir(breakData.pos)) {
                    MapleCrystal.BREAK.doubleMap.remove(i);
                    continue;
                }
                BreakManager.BreakData singleBreakData = MapleCrystal.BREAK.breakMap.get(i);
