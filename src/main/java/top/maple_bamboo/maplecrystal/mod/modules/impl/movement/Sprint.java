...
import top.maple_bamboo.maplecrystal.mod.modules.impl.player.Freecam;
...
extends Module {
    ...
    private boolean shouldSprint() {
        if (!(Sprint.mc.player.getHungerManager().getFoodLevel() <= 6 && !Sprint.mc.player.isCreative() || !MovementUtil.isMoving() || this.pause || Sprint.mc.player.isSneaking() && this.sneakingPause.getValue() || MapleCrystal.PLAYER.isInWeb((PlayerEntity)Sprint.mc.player) && this.inWebPause.getValue() || Sprint.mc.player.isUsingItem() && this.usingPause.getValue() || Sprint.mc.player.isRiding() || Sprint.mc.player.hasStatusEffect(StatusEffects.BLINDNESS) && this.blindnessPause.getValue())) {
            switch (this.mode.getValue().ordinal()) {
                case 1: {
                    if (AntiCheat.INSTANCE.movementSync()) {
                        return Sprint.mc.player.input.movementForward > 0.0f;
                    }
                    return HoleSnap.INSTANCE.isOn() || Sprint.mc.options.forwardKey.isPressed() && MathHelper.angleBetween((float)Sprint.mc.player.getYaw(), (float) MapleCrystal.ROTATION.rotationYaw) < 40.0f;
                }
                case 3: {
                    if (AntiCheat.INSTANCE.movementSync()) {
                        return Sprint.mc.player.input.movementForward > 0.0f;
                    }
                    return HoleSnap.INSTANCE.isOn() || MathHelper.angleBetween((float)Sprint.getSprintYaw(Sprint.mc.player.getYaw()), (float) MapleCrystal.ROTATION.rotationYaw) < 40.0f;
                }
                case 2: {
                    return true;
                }
