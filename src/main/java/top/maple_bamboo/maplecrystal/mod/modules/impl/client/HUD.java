...
import top.maple_bamboo.maplecrystal.api.utils.math.Animation;
import top.maple_bamboo.maplecrystal.api.utils.math.Easing;
...
import top.maple_bamboo.maplecrystal.asm.accessors.ISimpleRegistry;
...
extends Module {
    ...
    public HUD() {
        ...
        this.setChinese("\u754c\u9762");
        INSTANCE = this;
        Config.EVENT_BUS.subscribe(new InitHandler());
        ...
        this.infoList.add(new Info(() -> "TPS \u00a7f" + Config.SERVER.getTPS() + " [" + Config.SERVER.getCurrentTPS() + "]", this.tps::getValue));
        this.infoList.add(new Info(() -> {
        ...
        this.infoList.add(new Info(() -> "FPS \u00a7f" + Config.FPS.getFps(), this.fps::getValue));
    }
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        if (this.waterMark.getValue()) {
            if (this.pulse.booleanValue) {
                TextUtil.drawStringPulse(drawContext, this.waterMarkString.getValue().replaceAll("%version%", Config.VERSION).replaceAll("%hackname%", Config.NAME), this.waterMarkOffset.getValueInt(), this.waterMarkOffset.getValueInt(), this.color.getValue(), this.pulse.getValue(), this.pulseSpeed.getValue(), this.pulseCounter.getValueInt(), this.font.getValue(), this.shadow.getValue());
            } else {
                TextUtil.drawString(drawContext, this.waterMarkString.getValue().replaceAll("%version%", Config.VERSION).replaceAll("%hackname%", Config.NAME), this.waterMarkOffset.getValueInt(), this.waterMarkOffset.getValueInt(), this.color.getValue().getRGB(), this.font.getValue(), this.shadow.getValue());
            }
        }
...
//        }
    ...
    public class InitHandler {
        ...
        public void onInit(InitEvent event) {
            ...
            MapleCrystal.EVENT_BUS.unsubscribe(this);
        }
    }
            ...
            if (this.currentX > 0.0 || this.isOn) {
                ...
                if (fade > 0.04) {
                    ...
                    if (HUD.this.blur.getValue()) {
                        Config.BLUR.applyBlur((float)(HUD.this.radius.getValue() * fade), (float)((double)windowWidth - width - (double)(HUD.this.width.getValueFloat() / 2.0f)), (float)currentY - 1.0f - HUD.this.interval.getValueFloat() / 2.0f, (float)width + HUD.this.width.getValueFloat() - HUD.this.xOffset.getValueFloat(), (float)fontHeight + HUD.this.interval.getValueFloat());
                    }
