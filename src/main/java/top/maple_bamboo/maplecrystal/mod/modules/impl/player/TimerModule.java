    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        if (MovementUtil.isMoving() && !EntityUtil.isInsideBlock()) {
            ...
            if (this.timer2.passed(this.lastMs)) {
                ...
                double timer = (double) MapleCrystal.TIMER.getDefault() + (1.0 - this.end.ease(this.ease.getValue())) * (double)(this.shiftTimer.getValueFloat() - 1.0f) * ((double)this.lastMs / this.accumulate.getValue());
                MapleCrystal.TIMER.set((float)Math.max((double) MapleCrystal.TIMER.getDefault(), timer));
            } else {
                MapleCrystal.TIMER.set(this.shiftTimer.getValueFloat());
            }
        } else {
            if (this.moving) {
                MapleCrystal.TIMER.reset();
