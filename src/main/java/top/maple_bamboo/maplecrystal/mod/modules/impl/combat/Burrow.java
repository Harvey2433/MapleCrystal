...
extends Module {
    ...
    public void onUpdate(ClientTickEvent event) {
        ...
        if (event.isPost()) {
            ...
            if (MapleCrystal.PLAYER.isInWeb((PlayerEntity)Burrow.mc.player)) {
                this.webTimer.reset();
                return;
            }
            ...
            if (this.rotate.getValue() == RotateMode.Bypass) {
                if (above) {
                    ...
                    MapleCrystal.ROTATION.snapAt(angle[0], angle[1]);
                } else {
    ...
    private boolean canPlace(BlockPos pos) {
        ...
        if (this.detectMine.getValue() && MapleCrystal.BREAK.isMining(pos)) {
            return false;
        }
