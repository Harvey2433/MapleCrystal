...
import top.maple_bamboo.maplecrystal.api.events.impl.EntityVelocityUpdateEvent;
...
import top.maple_bamboo.maplecrystal.api.events.impl.UpdateRotateEvent;
...
extends Module {
    ...
    public void onRotate(UpdateRotateEvent event) {
        if (this.noRotation()) {
            event.setRotation(MapleCrystal.ROTATION.rotationYaw, 89.0f);
        }
    }
    ...
    public void onUpdate(TickEvent event) {
        ...
        if (this.flag) {
            if (this.lagBackTimer.passedMs(this.lagPause.getValue()) && (this.flagInWall.getValue() && (!pushOutOfBlocks || this.whenPushOutOfBlocks.getValue()) || !EntityUtil.isInsideBlock())) {
                mc.getNetworkHandler().sendPacket((Packet)new PlayerMoveC2SPacket.Full(Velocity.mc.player.getX(), Velocity.mc.player.getY(), Velocity.mc.player.getZ(), MapleCrystal.ROTATION.rotationYaw, MapleCrystal.ROTATION.rotationPitch, Velocity.mc.player.isOnGround()));
