...
import top.maple_bamboo.maplecrystal.api.utils.render.ModelPlayer;
...
import top.maple_bamboo.maplecrystal.mod.modules.impl.client.TextRadar;
...
extends Module {
    ...
    public void onRender3D(MatrixStack matrixStack) {
        for (ModelPlayer data : this.logoutCache.values()) {
            ...
            Render3DUtil.drawText3D(player.getName().getString() + (String)(this.health.getValue() ? String.valueOf(TextRadar.getHealthColor(player)) + " " + LogoutSpots.round2(player.getHealth() + player.getAbsorptionAmount()) : "") + (String)(this.totem.getValue() && MapleCrystal.POP.getPop(player) > 0 ? String.valueOf(TextRadar.getPopColor(MapleCrystal.POP.getPop(player))) + " -" + MapleCrystal.POP.getPop(player) : ""), new Vec3d(player.getX(), ((IEntity)player).getDimensions().getBoxAt((Vec3d)player.getPos()).maxY + 0.5, player.getZ()), this.text.getValue());
        }
    }
