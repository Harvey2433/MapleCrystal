...
extends Module {
    ...
    Type isHole(BlockPos pos) {
        if (HoleESP.mc.world.isAir(pos) && (!this.airYCheck.getValue() || pos.getY() == HoleESP.mc.player.getBlockY() - 1 || pos.getY() == HoleESP.mc.player.getBlockY()) && MapleCrystal.HOLE.isHard(pos.up())) {
            return Type.Air;
        }
        int blockProgress = 0;
        boolean bedRock = true;
        for (Direction i : Direction.values()) {
            if (i == Direction.UP || i == Direction.DOWN || !MapleCrystal.HOLE.isHard(pos.offset(i))) continue;
        ...
        if (MapleCrystal.HOLE.isDoubleHole(pos)) {
            return Type.Normal;
        }
        return Type.None;
    }
