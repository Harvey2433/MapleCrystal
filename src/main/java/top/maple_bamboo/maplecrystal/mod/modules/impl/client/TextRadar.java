...
extends Module {
    ...
    public void onRender2D(DrawContext drawContext, float tickDelta) {
        ...
        for (PlayerEntity playerEntity : players) {
            ...
            if (this.pops.getValue() && (totemPopped = MapleCrystal.POP.getPop(playerEntity)) > 0) {
                stringBuilder.append(blank);
                stringBuilder.append(TextRadar.getPopColor(totemPopped));
                stringBuilder.append("-");
                stringBuilder.append(totemPopped);
            }
            if ((isFriend = MapleCrystal.FRIEND.isFriend(playerEntity)) && !this.friend.booleanValue) continue;
