...
import top.maple_bamboo.maplecrystal.api.interfaces.IChatHudHook;
import top.maple_bamboo.maplecrystal.api.utils.Wrapper;
import top.maple_bamboo.maplecrystal.mod.commands.Command;
import top.maple_bamboo.maplecrystal.mod.commands.impl.*;
...
implements Wrapper {
    ...
    public void command(String[] commandIn) {
        Command command = this.commands.get(commandIn[0].substring(MapleCrystal.getPrefix().length()).toLowerCase());
        if (command == null) {
