...
import top.maple_bamboo.maplecrystal.api.events.impl.BlockBreakingProgressEvent;
...
import top.maple_bamboo.maplecrystal.api.events.impl.ServerConnectBeginEvent;
...
import top.maple_bamboo.maplecrystal.mod.modules.impl.render.BreakESP;
...
implements Wrapper {
    ...
    public BreakManager() {
        MapleCrystal.EVENT_BUS.subscribe(this);
    }
    ...
    public void onTick(ClientTickEvent event) {
        ...
        if (AntiCheat.INSTANCE.detectDouble.getValue()) {
            Iterator<Object> iterator = ((ConcurrentHashMap.KeySetView) MapleCrystal.BREAK.doubleMap.keySet()).iterator();
            while (iterator.hasNext()) {
                int i = (Integer)iterator.next();
                BreakData breakData = MapleCrystal.BREAK.doubleMap.get(i);
    ...
    public void onPacket(PacketEvent.Receive event) {
        ...
        if (packet instanceof BlockBreakingProgressS2CPacket) {
            ...
            MapleCrystal.EVENT_BUS.post(BlockBreakingProgressEvent.get(packet2.getPos(), packet2.getEntityId(), packet2.getProgress()));
